...
const _PROD_LIST_COLS = 'id,name,price,old_price,store,cat,badge,bg,emoji,img_url,description,brand,specs,stock,variants,barcode,created_at';
// Fallback for DBs that haven't run the `barcode` column migration yet (schema.sql)
const _PROD_LIST_COLS_NO_BARCODE = 'id,name,price,old_price,store,cat,badge,bg,emoji,img_url,description,brand,specs,stock,variants,created_at';

async function fetchProducts() {
  try {
    let data;
    try {
      data = await _rest('GET', `products?select=${_PROD_LIST_COLS}&order=created_at.asc`);
    } catch (e) {
      if (e.message.includes('42703') || e.message.includes('barcode')) {
        data = await _rest('GET', `products?select=${_PROD_LIST_COLS_NO_BARCODE}&order=created_at.asc`);
      } else {
        throw e;
      }
    }
...
async function fetchProductById(id) {
  ...
  try {
    let data;
    try {
      data = await _rest('GET', `products?id=eq.${encodeURIComponent(id)}&select=${_PROD_LIST_COLS}`);
    } catch (e) {
      if (e.message.includes('42703') || e.message.includes('barcode')) {
        data = await _rest('GET', `products?id=eq.${encodeURIComponent(id)}&select=${_PROD_LIST_COLS_NO_BARCODE}`);
      } else {
        throw e;
      }
    }
...
async function saveProduct(product) {
  ...
  try {
    ...
    if (e.message.includes('42703') || e.message.includes('column')) {
      const { stock, variants, images, brand, specs, barcode, ...fallback } = row;
      try {
